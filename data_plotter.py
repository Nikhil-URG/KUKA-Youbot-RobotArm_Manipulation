# data_plotter_2d_separate_plots.py
# Creates 9 beautiful individual plots — one per size+direction
...
# Plot raw data (no outlier removal)
...
for size in sorted(matched['size'].unique()):
    ...
    for direction in ['straight', 'left', 'right']:
        ...
        # YouBot (internal) - single point per direction
        if not group.empty:
            ...
            plt.scatter(youbot_x, youbot_y, c=colors[direction], s=200, marker=markers[direction],
                       label=f"YouBot {direction}", edgecolors='black', linewidth=1.5, zorder=10)
...
preprocessed_df.to_csv(ROOT / "preprocessed_data.csv", index=False)
print(f"\nSaved preprocessed data to {ROOT / 'preprocessed_data.csv'}")

print("\nAll 3 combined plots saved successfully!")